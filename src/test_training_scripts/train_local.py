...
LEARNING_RATE = 3e-5
NUM_EPOCHS = 10  # Train for more epochs to see learning trends
# ** CRITICAL for 4GB VRAM **
# Use a small batch size to fit in memory
PER_DEVICE_BATCH_SIZE = 4
# Use gradient accumulation to simulate a larger batch size (4x4=16 effective batch size)
# This improves training stability without increasing memory usage.
...
def load_and_prepare_data(data_folder):
    ...
    full_df.rename(columns={'English': 'english', 'Dagbani': 'dagbani'}, inplace=True, errors='ignore')
    
    # --- THIS IS THE CORRECTED SECTION ---
    # Cleaning (updated)
    # First, ensure all data is treated as strings using .astype(str)
    # Then, apply the string functions.
...
def main():
    ...
    # --- Step 3: Define Metrics for Analytics (UPDATED) ---
    ...
    # --- Step 4: Configure and Run Training (UPDATED) ---
    ...
    print("\n--- All Done! ---")
    print(f"You can now use the `translate.py` script with the model saved in the '{MODEL_OUTPUT_DIR}' folder.")
